...
public class BinarySearchUni {
    ...
    Post: index, min i, a[i - 1] < a[i] <= a[i + 1]
    */
    public static void main(String[] args) {
        ...
        if (sum % 2 == 0) {
            System.out.println(recursiveBinarySearch(-1, a.length - 1, a));
        } else {
    ...
    */
    public static int recursiveBinarySearch(int left, int right, int[] a) {
        ...
        // left < mid < right && a[mid] - int && a[mid + 1] - int
        if (comparator(a[mid], a[mid + 1])) {
            // a[mid] > x && left <= index <= right && forall (i, j in [1, args.length)) i < j => args[i] >= args[j]
            right = mid;
            // left' == mid
        ...
        // [left', right'] является подмножеством [left, right], следовательно наш отрезок уменьшился.
        // в силу того, что mid находится в [left, right], то мы в конце концов придем к left + 1 == right
        return recursiveBinarySearch(left, right, a);
        // index, forall (i, a[i] <= x) index <= i
    }
}
