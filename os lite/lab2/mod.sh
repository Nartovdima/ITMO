...
uid=$(awk -F ':' '{if ($1 == $username) print $3}' /etc/passwd)

max_memory_usage=0
pid=-1
top_result=$(top -b -n1 -u $username -o VIRT | sed 1,7d | head -n1 | awk '{print $1}')
for process in /proc/*; do
        if [[ -f $process/statm ]] && [[ -f $process/status ]] && [[ $(awk '{if ($1 == "Uid:") print $2}' $process/status) -eq $uid ]] ; then
