...
struct ST {
    ...
    ll get_sum (int v) {
        if (tree[v].push == 0)
            ...
            return tree[v].push + tree[v].sum;
    }
 
    void relax (int v) {
        if (v < sz) 
            tree[v].sum = max(get_sum(v * 2), get_sum(v * 2 + 1));
    }
 
    void push (int v) {
        if (v < sz && tree[v].push != 0) {
            //cout << "push vertex " << v << '\n';
    ...
    ll get_f (int v, int l, int r, int L, int R) {
        ...
        push(v);
        int m = (l + r) / 2;
        return max(get_f(v * 2, l, m, L, R), get_f(v * 2 + 1, m, r, L, R));
    } 
...
signed main() {
    ...
    while (m--) {
        ...
        a--;
        if (c == 'm') {
            cout << tmp.get_f(1, 0, n, a, b) << ' ';
        } else {
            ll x;
            cin >> x;
            tmp.update(1, 0, n, a, b, x);
            //tmp.print(1, 0, n);
            //cout << '\n';
        }   
    }
