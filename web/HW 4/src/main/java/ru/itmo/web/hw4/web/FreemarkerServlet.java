...
public class FreemarkerServlet extends HttpServlet {
    ...
    private void pageNotFound(HttpServletRequest request, HttpServletResponse response) throws IOException {
        response.setStatus(HttpServletResponse.SC_NOT_FOUND);
        Template template = freemarkerConfiguration.getTemplate("/errorPage.ftlh");
        try {
            Map<String, Object> data = new HashMap<>();
            data.put("errorCode", HttpServletResponse.SC_NOT_FOUND);
            data.put("errorMessage", "No such page");
            data.put("currentPage", request.getRequestURI());
            template.process(data, response.getWriter());
        } catch (TemplateException e) {
            e.printStackTrace();
