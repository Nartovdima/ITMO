...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Map<Class<?>, Object> types2instances = Map.of(
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            method = (Arrays.stream(clazz.getDeclaredMethods())
                    .filter((listedMethod) -> (listedMethod.getName().equals(route.action)
                            && Arrays.stream(listedMethod.getParameterTypes()).allMatch(types2instances::containsKey)))
        ...
        try {
            Object[] params = Arrays.stream(method
                            .getParameterTypes())
                            .map(types2instances::get).toArray();
    ...
    private Template newTemplate(String templateName) throws ServletException, TemplateNotFoundException {
        ...
        if (template == null) {
            throw new TemplateNotFoundException(templateName, null, "No such template: " + templateName);
        }
